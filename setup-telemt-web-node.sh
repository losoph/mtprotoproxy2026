...
# ME (middle-proxy) mode: a different upstream path to Telegram, for when the
# node's direct DC addresses are filtered. See docs/upstream-path.md — including
# why MIDDLE_PROXY_NAT_IP matters when the provider egresses from another address.
USE_MIDDLE_PROXY="${USE_MIDDLE_PROXY:-0}"
MIDDLE_PROXY_NAT_IP="${MIDDLE_PROXY_NAT_IP:-}"   # empty => PUBLIC_IP when ME is on
AD_TAG="${AD_TAG:-}"                             # 32 hex from @MTProxybot; ME-only
...
# ---- config -------------------------------------------------------------------
log "config (web host: $WEB_HOST, carrier: $WEB_CARRIER, mtproto: $([ "$KEEP_MTPROTO" = 1 ] && echo "$DOMAIN:$PORT" || echo off), me: $USE_MIDDLE_PROXY)"
if [ "$USE_MIDDLE_PROXY" = 1 ]; then
  ME_ENABLED=true
  # ME key derivation uses the address Telegram sees. Where that differs from the
  # interface address (a provider NAT or egress pool, as on tldw), leaving this
  # unset fails in ways that point nowhere near the cause.
  ME_EXTRA="middle_proxy_nat_ip = \"${MIDDLE_PROXY_NAT_IP:-$PUBLIC_IP}\""
  [ -n "$AD_TAG" ] && ME_EXTRA="$ME_EXTRA
ad_tag = \"$AD_TAG\""
else
  ME_ENABLED=false
  ME_EXTRA=""
